...
use gyu_model::{address::Address, utilities::to_hex_string, AddressError, PrivateKey};
...
mod tests {
    ...
    fn test_from_private_key(expected_address: &str, private_key: &EthereumPrivateKey) {
        let address =
            EthereumAddress::from_private_key(private_key, &EthereumFormat::Standard).unwrap();
        assert_eq!(expected_address, address.to_string());
    }

    fn test_from_public_key(expected_address: &str, public_key: &EthereumPublicKey) {
        let address =
            EthereumAddress::from_public_key(public_key, &EthereumFormat::Standard).unwrap();
        assert_eq!(expected_address, address.to_string());
    }
    ...
    fn test_checksum_address_invalid() {
        ...
        let address =
            "0x9141B7539E7902872095C408BfA294435e2b8c8a0x9141B7539E7902872095C408BfA294435e2b8c8a";
        assert!(EthereumAddress::from_str(address).is_err());
    }
}
