...
pub trait Transaction: 'static + Clone + Send + Sync {
...
pub enum TransactionError {
    ...
    #[fail(
        display = "insufficient information to craft transaction. missing: {}",
        _0
    )]
    ...
    #[fail(
        display = "invalid transaction RLP length: expected - 9, found - {:?}",
        _0
    )]
    ...
    #[fail(
        display = "invalid transaction - either both sender and signature should be present, or neither"
    )]
...
impl From<crate::no_std::io::Error> for TransactionError {
    fn from(value: crate::no_std::io::Error) -> Self {
        TransactionError::Crate("crate::no_std::io", format!("{:?}", value))
    }
}

impl From<&'static str> for TransactionError {
    fn from(value: &'static str) -> Self {
        TransactionError::Message(value.into())
    }
}

impl From<()> for TransactionError {
    fn from(value: ()) -> Self {
        TransactionError::NullError(value)
    }
}
